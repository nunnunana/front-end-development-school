...
#### @import를 일반적으로 쓰는 것과 네임스페이스 안족에 중첩으로 쓰는 것의 차이
코드가 생성
...
동일한 파일 내에서는 사실상 의미없으나, 분리된 파일에서 각각의 변수를 사용하는 경우에서 활용가능한 개념.

선언된 변수값이 없을 때는 default값을 가져와서 사용하게 됨. 그래서 우선권을 누가 가지는 것인지?
